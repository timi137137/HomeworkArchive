...
//
//status CreateSeqList(SeqList<int>& seqList, int Maxsize) {
//	int i, input;
//	for (i = 0; i < Maxsize; i++) {
//		cin >> input;
...
//}
//
//status SelectionSort(SeqList<int>& seqList) {
...
//}
//
//status BubbleSort(SeqList<int>& seqList) {
//	if (seqList.isEmpty()) return OVERFLOW;
//
//	int n = seqList.getLength();
//
...
//}
//
//status InsertionSort(SeqList<int>& seqList) {
...
//
///* 选取三个元素的中位数 */
//int getMedian(SeqList<int>& seqList, int left, int mid, int right) {
//	// 此处使用异或运算来简化代码
...
//}
///* 哨兵划分（三数取中值） */
//int partition(SeqList<int>& seqList, int left, int right) {
//	// 选取三个候选元素的中位数
...
//	return i;            // 返回基准数的索引
//}
//status QuickSort(SeqList<int>& seqList, int left, int right) {
//	// 数组长度为 1 时终止
//	if (left < right) {
//		// 哨兵划分操作
...
//	for (int i = 0; i < 4; i++)
//	{
//		SeqList<int> seqList_temp(seqList);
